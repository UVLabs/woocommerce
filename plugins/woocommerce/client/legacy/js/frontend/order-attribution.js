( function ( wc_order_attribution ) {
	...
	function setFormValues( values ) {
		...
		if( document.querySelector( `input[name^="${params.prefix}"]` ) ) {
			for( const key of Object.keys( wc_order_attribution.fields ) ) {
				document.querySelector( `input[name="${params.prefix}${key}"]` ).value = values?.[ key ];
			}
		}
	};
	/**
	...
	// Run init.
	setOrderTracking( params.allowTracking );
